...
static Tensor mps_convolution_backward_input(IntArrayRef input_size,
  ...
  bool is_channels_last = (output_memory_format == kChannelsLast) && !is3DConv;
  auto grad_input_t =
...
static Tensor mps_convolution_backward_weights(IntArrayRef weight_size,
  ...
  @autoreleasepool {
    ...
    // Feed the input in the output's layout: the kernel emits grad_weight in
    // whatever format the input is given.
    auto input_feed = is_channels_last ? input_t.contiguous(kChannelsLast) : input_t;
    auto gradOutputPlaceholder =
        Placeholder(cachedGraph->gradOutputTensor_, grad_weight_c ? grad_output_t.contiguous() : grad_output_t);
    auto inputPlaceholder = Placeholder(cachedGraph->inputTensor_, grad_weight_c ? input_t.contiguous() : input_feed);
...
std::tuple<at::Tensor, at::Tensor, at::Tensor> mps_convolution_backward(const at::Tensor& input,
  ...
  if (input.numel() == 0) {
    ...
    const auto memory_format =
        mps_conv_use_channels_last(input, weight) ? at::MemoryFormat::ChannelsLast : at::MemoryFormat::Contiguous;
    if (output_mask[0]) {
...
static Tensor mps_convolution_transpose_forward(const Tensor& grad_output,
  ...
  const auto output_memory_format =
      mps_conv_use_channels_last(grad_output, weight) ? at::MemoryFormat::ChannelsLast : at::MemoryFormat::Contiguous;
...
static Tensor mps_convolution_transpose_backward_weight(IntArrayRef weight_size,
                                                        ...
                                                        IntArrayRef dilation,
                                                        int64_t groups) {
  const auto output_memory_format = mps_conv_use_channels_last(input_t, grad_output_t) ? at::MemoryFormat::ChannelsLast
                                                                                       : at::MemoryFormat::Contiguous;
